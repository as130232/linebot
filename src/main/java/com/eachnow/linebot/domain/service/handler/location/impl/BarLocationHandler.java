...
@Component
public class BarLocationHandler implements LocationHandler {
    private final Integer MAX_SIZE = 10;
    private GoogleApiService googleApiService;

    @Autowired
    public BarLocationHandler(GoogleApiService googleApiService) {
        this.googleApiService = googleApiService;
    }

    @Override
    public Message execute(LocationMessageContent content) {
        ResultLocationPO resultLocationPO = googleApiService.getLocation(String.valueOf(content.getLatitude()), String.valueOf(content.getLongitude()), GooglePlaceTypeEnum.BAR, LanguageEnum.TW.getLang());
        ...
        return new TemplateMessage("酒館精選", carouselTemplate);
    }
